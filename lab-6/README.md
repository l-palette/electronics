# Работа №6: Исследование JK-триггера и счетчика

## Цель работы

Изучение работы триггеров и двоичного асинхронного суммирующего счетчика

# Упражнение 1. Исследование JK-триггера

## Схема соединений JK-триггера

![scheme](_assets/img_1.png)

JK-триггер имеет входы:
...
## 1.1. Составление таблицы истинности JK-триггера
...
## 1.2. Исследование JK-триггера в счётном режиме
...
Осциллограммы сигналов $C(t)$, $Q(t)$ и $Q̅(t)$ на одном графике, синхронно друг под другом:
...
# Упражнение 2. Исследование асинхронного четырёхразрядного двоичного счётчика
...
## 2.1. Таблица состояний асинхронного четырёхразрядного двоичного счётчика
Состояние выходных разрядов счётчика ($Q_1$, $Q_2$, $Q_3$, $Q_4$) определяем по свечению светодиодов

| № импульса C |$Q_4$ | $Q_3$ | $Q_2$ | $Q_1$ | Двоичное число |
|----|----|---|---|---|---|
| 0 | 0 | 0 | 0 | 0 | 0000 |
| 1 | 0 | 0 | 0 | 1 | 0001 |
| 2 | 0 | 0 | 1 | 0 | 0010 |
...
## 2.2. Исследование работы счётчика с помощью осциллографа
...
В ходе лабораторной работы были исследованы элементы последовательностной логики: JK-триггер и асинхронный четырёхразрядный двоичный суммирующий счётчик.

Было установлено, что JK-триггер может работать в режимах хранения, установки, сброса и переключения. При комбинации входных сигналов J = 1 и K = 1 триггер переходит в счётный режим и изменяет своё состояние на противоположное при каждом активном фронте тактового сигнала.

Исследованный четырёхразрядный асинхронный двоичный счётчик последовательно формирует состояния от 0000 до 1111, после чего возвращается в начальное состояние 0000. Следовательно, счётчик работает по модулю 16.

На осциллограмме было подтверждено, что каждый следующий разряд счётчика переключается в два раза реже предыдущего. Наличие задержки между переключениями разрядов показывает асинхронный принцип работы счётчика.

Таким образом, в работе были подтверждены основные свойства JK-триггера и асинхронного двоичного счётчика, применяемых для хранения информации, деления частоты и счёта импульсов.
